...
all_sap <- simpleSelection(df=all_fungi, guild= "Saprotroph", family="NA", block= c("F","N") ,focalcrop= c("Eggplant"), farmtype=c("Monoculture","Polyculture"), year = c("2017"), env_factors=envi_factors, geo=TRUE, maxDist = "both")

all_fun <- simpleSelection(df=all_fungi, guild= "all fungi", family="NA", block= c("F","N") ,focalcrop= c("Eggplant"), farmtype=c("Monoculture","Polyculture"), year = c("2017"), env_factors=envi_factors, geo=TRUE, maxDist = "both")
...
all_wa <- simpleSelection(df=all_fungi, guild= "Arbuscular Mycorrhizal",  family="NA", block= c("F","N") ,focalcrop= c("Eggplant"), farmtype=c("Monoculture","Polyculture"), year = c("2018","2017"), env_factors=envi_factors, geo=TRUE, maxDist = "both")
...
# Complete
predictorsDF_cd <- rbind(all_predictorsDF, mono_predictorsDF, poly_predictorsDF) %>%
  mutate(location = factor(location, levels = c("whole farm", "within rows", "across rows")),
         dataset = factor(dataset, levels=c("all farms","monoculture farms", "polyculture farms")))

## *****************************************************************************
## With Farm type not crop Diversity #########################################################
## *****************************************************************************

envi_factors_ft <- c("pH", "P", "TOC", "N","FarmBi")
...
# within and across rows
all_wa_ft <- simpleSelection(df=all_fungi, guild= "Arbuscular Mycorrhizal",  family="NA", block= c("F","N") ,focalcrop= c("Eggplant"), farmtype=c("Monoculture","Polyculture"), year = c("2018","2017"), env_factors=envi_factors_ft, geo=TRUE, maxDist = "both")

# df <- all_wa_ft$df %>%
#   filter(! FarmKey %in% c("DS","GA")) 
# 
# species_table <- df %>%
#   dplyr::select(contains("OTU")) 
# 
# 
# dist.sp <- vegdist(species_table, "jaccard")
# 
# dispersion <- betadisper(dist.sp, df$FarmType,  type = "median")
# dispersionTest <-  permutest(dispersion)
# dispersionTest
# boxplot(dispersion)
# 
# species_matrix <- cbind(all_wa_ft$Key, dist.sp) 

# within
all_w_ft <- simpleSelection(df=all_fungi, guild= "Arbuscular Mycorrhizal",  family="NA", block= c("F") ,focalcrop= c("Eggplant"), farmtype=c("Monoculture","Polyculture"), year = c("2018","2017"), env_factors=envi_factors_ft, geo=TRUE, maxDist = "both")

# across
all_a_ft <- simpleSelection(df=all_fungi, guild= "Arbuscular Mycorrhizal",  family="NA", block= c("N") ,focalcrop= c("Eggplant"), farmtype=c("Monoculture","Polyculture"), year = c("2018","2017"), env_factors=envi_factors_ft, geo=TRUE, maxDist = "both")
...
# within and across rows
mono_wa_ft <- simpleSelection(df=all_fungi, guild= "Arbuscular Mycorrhizal",  family="NA", block= c("F","N") ,focalcrop= c("Eggplant"), farmtype=c("Monoculture"), year = c("2018","2017"), env_factors=envi_factors_ft, geo=TRUE, maxDist = "both")

# within
mono_w_ft <- simpleSelection(df=all_fungi, guild= "Arbuscular Mycorrhizal",  family="NA", block= c("F") ,focalcrop= c("Eggplant"), farmtype=c("Monoculture"), year = c("2018","2017"), env_factors=envi_factors_ft, geo=TRUE, maxDist = "both")

# across
mono_a_ft <- simpleSelection(df=all_fungi, guild= "Arbuscular Mycorrhizal",  family="NA", block= c("N") ,focalcrop= c("Eggplant"), farmtype=c("Monoculture"), year = c("2018","2017"), env_factors=envi_factors_ft, geo=TRUE, maxDist = "both")
...
# within and across rows
poly_wa_ft <- simpleSelection(df=all_fungi, guild= "Arbuscular Mycorrhizal",  family="NA", block= c("F","N") ,focalcrop= c("Eggplant"), farmtype=c("Polyculture"), year = c("2018","2017"), env_factors=envi_factors_ft, geo=TRUE, maxDist = "both")

# within
poly_w_ft <- simpleSelection(df=all_fungi, guild= "Arbuscular Mycorrhizal",  family="NA", block= c("F") ,focalcrop= c("Eggplant"), farmtype=c("Polyculture"), year = c("2018","2017"), env_factors=envi_factors_ft, geo=TRUE, maxDist = "both")

# across
poly_a_ft <- simpleSelection(df=all_fungi, guild= "Arbuscular Mycorrhizal",  family="NA", block= c("N") ,focalcrop= c("Eggplant"), farmtype=c("Polyculture"), year = c("2018","2017"), env_factors=envi_factors_ft, geo=TRUE, maxDist = "both")
...
# ALL FARMS
all_predictorsDF_ft <- rbind(
  all_wa_ft$predictors %>% mutate(dataset= "all farms", location = "whole farm"),
  all_w_ft$predictors %>% mutate(dataset= "all farms", location = "within rows"),
  all_a_ft$predictors %>% mutate(dataset= "all farms", location = "across rows")) %>%
  mutate(location = factor(location, levels = c("whole farm", "within rows", "across rows")))


# MONOCULTURE
mono_predictorsDF_ft <- rbind(
  mono_wa_ft$predictors %>% mutate(dataset= "monoculture farms", location = "whole farm"),
  mono_w_ft$predictors %>% mutate(dataset= "monoculture farms", location = "within rows"),
  mono_a_ft$predictors %>% mutate(dataset= "monoculture farms", location = "across rows")) %>%
  mutate(location = factor(location, levels = c("whole farm", "within rows", "across rows")))


# POLYCULTURE
poly_predictorsDF_ft <- rbind(
  poly_wa_ft$predictors %>% mutate(dataset= "polyculture farms", location = "whole farm"),
  poly_w_ft$predictors %>% mutate(dataset= "polyculture farms", location = "within rows"),
  poly_a_ft$predictors %>% mutate(dataset= "polyculture farms", location = "across rows")) %>%
  mutate(location = factor(location, levels = c("whole farm", "within rows", "across rows")))


# Complete
predictorsDF_ft <- rbind(all_predictorsDF_ft, mono_predictorsDF_ft, poly_predictorsDF_ft) %>%
  mutate(location = factor(location, levels = c("whole farm", "within rows", "across rows")),
         dataset = factor(dataset, levels=c("all farms","monoculture farms", "polyculture farms")))
