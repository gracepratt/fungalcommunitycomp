...
final_ispline <- ggplot(predictorsDF, aes(x = X, y = Y, color = Factor)) +
  geom_line(size=1) +
  xlab("Standardized Variables") +
  ylab("Partial Ecological Distance") + 
  scale_color_manual(values=predictorColors) + 
  scale_y_continuous(expand = c(0.01, 0.01), breaks=round(seq(min(predictorsDF$Y), max(predictorsDF$Y), length.out = 6),1)) +
  # ylim(0, 2.5) +
...
final_varImp_plot <- ggplot(varImp %>% mutate(location = factor(location, levels = c("whole farm","within rows","across rows"))), aes(fill=predictor, y=variance, x=location)) + 
  geom_bar(position="stack", stat="identity") +
  ylab("% variance explained") +
  xlab("spatial scale") + 
  scale_fill_manual(values = predictorColors) +
...
envDF <- all %>% dplyr::select(cropDiversity, N, P, pH, TOC)
ordDF <- composition$dist
otus <- all %>% dplyr::select(contains("OTU"))
