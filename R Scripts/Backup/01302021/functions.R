...
predictors_plot <- function(model){
  ...
  palette <- data.frame(Predictors = c("cropDiversity","Geographic","N", 
                                       "P", "pH", "TOC", "PolyYears"), 
                        colors = c("#E69F00", "#009E73", "#8EB042",
...
relAbunPlot <- function(data, variable, taxanomy = tax){
  ...
  otu.pa <- decostand(data %>% dplyr::select(contains("OTU")), "pa")  
  ...
  # plot
  relAbunPlot <-  ggplot(relAbunDF, aes(fill=Taxon, y=freq, x=factor(round_any(var,roundFactor)))) +
    geom_bar(position="fill", stat="identity") +
    ylab("relative abundance") +
    xlab(variable) +
    scale_fill_manual(values= taxonColor(relAbunDF$Taxon))+
    theme_classic() + theme(axis.text.x = element_text(angle = 45, hjust=1)) + themeBorder
...
pcoaFun <- function(data, shape, color, formula = NULL, strata= NULL, plot.means = FALSE){
  ...
  plot <- ggplot(data=df, aes(x=Axis.1,y=Axis.2, colour= clr)) + 
    geom_point(aes(x=Axis.1,y=Axis.2), size=4, stroke=1, alpha=0.75)+ 
    # scale_color_manual(values=c('#dfc27d', '#018571')) +
...
run.MMMR <- function(data, vars, trans=FALSE,type="bray"){
  ...
  if(trans == "hellinger"){
    otu <- decostand(otu <- df %>% dplyr::select(contains("OTU")),"hellinger")
  }else {
    otu <- df %>% dplyr::select(contains("OTU"))
  }
  ...
  # CREATE DISSIMILARITY MATRIX FOR OTUs
  if(type == "nestedness"){
    dist.otu<- beta.pair(decostand(otu, "pa"), index.family = "jaccard")$beta.jne
  } else if( type == "turnover"){
    dist.otu<- beta.pair(decostand(otu, "pa"), index.family = "jaccard")$beta.jtu
  } else {
  ...
  for(i in vars){
    ...
    Xmats[[i]] <- as.matrix(dist(df %>% dplyr::select(i)))
  ...
  MMRRoutput <- MMRR(as.matrix(dist.otu),Xmats,nperm=999)
