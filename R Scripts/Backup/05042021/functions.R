...
alpha_predictor <- function(model, exp.var, color, raw.data = FALSE, length.out = 200){
  ...
  for (i in exp.var) {
    ...
    finalDF <- rbind(em_df_interaction)
...
ggplot(df, aes(x=factor1, y= variable, color = factor2, fill = factor2)) +
  geom_boxplot() +
  theme_bw() #from ggthemes package I think
  scale_color_manual(values=c('#dfc27d',  '#018571'))+
  scale_fill_manual(values=c('#dfc27d',  '#018571'))+
...
pcoaFun <- function(data, shape, color, formula = NULL, strata= NULL, plot.means = FALSE){
  ...
  ## ************ Plot ************ ##
  plot <- ggplot(data=df, aes(x=Axis.1,y=Axis.2)) + 
    # stat_ellipse(geom = "polygon", alpha=0.3, aes(fill=clr))+
    geom_point(aes(x=Axis.1,y=Axis.2, colour= clr), size=2.5, stroke=0, alpha=1)+ 
    scale_color_manual(values=c("#C10349","#1E88E5")) +
    scale_fill_manual(values=c("#C10349","#1E88E5")) +
  ...
  # legend_topRight()
    ...
    ## ************ Plot ************ ##
    plot <- ggplot(data=df, aes(x=Axis.1_mean,y=Axis.2_mean)) + 
      stat_ellipse(geom = "polygon", alpha=0.3, aes(fill=clr))+
      geom_errorbar(aes(ymin = Axis.2_mean - Axis.2_SE, ymax = Axis.2_mean + Axis.2_SE, colour= clr), 
                    width = 0) +
      geom_errorbar(aes(xmin = Axis.1_mean - Axis.1_SE, xmax = Axis.1_mean + Axis.1_SE, colour= clr),
                    width = 0) +
      geom_point(aes(x=Axis.1_mean,y=Axis.2_mean, colour= clr), size=2.5, stroke=0, alpha=1, fill = "white")+ 
      scale_color_manual(values=c('#b58648', '#017763')) +
      scale_fill_manual(values=c('#fccd47','#22b573')) +
