...
dbrda.test <- capscale(decostand(amf_otu,"hellinger")~ FarmBi +  N+ P+ pH+ TOC+ pcnmDF$PCNM1 , data=df, distance = "bray", sqrt.dist = TRUE)

dbrda.test <- capscale(decostand(amf_otu,"hellinger") ~ FarmBi +  N+ P+ pH+ TOC+dbmemDF$MEM1 + dbmemDF$MEM2 + dbmemDF$MEM3 + dbmemDF$MEM4 + dbmemDF$MEM5 + dbmemDF$MEM6 , data=df, distance = "bray", sqrt.dist = TRUE)

vif.cca(dbrda.test)
finalmodel <- ordistep(dbrda.test, scope=formula(dbrda.test), direction = "both")
...
anova.cca(finalmodel, by="terms", permutations = h)
